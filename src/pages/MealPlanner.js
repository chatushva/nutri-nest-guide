...
function MealPlanner() {
  ...
  const mealOptions = {
    ...
    lunch: [
      'Andhra Rice with Dal',
      'Vegetable Biryani',
      'Ragi Mudde with Curry',
      'Quinoa with Vegetables',
      'Brown Rice with Rasam'
    ],
    dinner: [
      'Andhra Fish Curry with Rice',
      'Vegetable Kurma with Roti',
      'Dal with Chapati',
  ...
  const handleMealSelect = (mealType, value) => {
  ...
  const getTotalCalories = () => {
    ...
    return Object.entries(selectedMeals)
      .filter(([_, meal]) => meal !== '')
      .reduce((total, [type, _]) => total + calorieEstimates[type], 0);
  };
